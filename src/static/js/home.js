...
const updateTitle = () => {
    if (networkName === '') {
        document.querySelector('#title').innerHTML = 'You are on worng network please select bscTestnet or RopstenTestnet.'
    } else {
...
const onInitSelect = async () => {
    ...
    if(networkName === 'RopstenTestnet'){
        bridgeSelecter.innerHTML = `
        <option selected value="bscTestnet">bscTestnet</option>
        `
    }else {
        bridgeSelecter.innerHTML = `
        <option selected value="RopstenTestnet">RopstenTestnet</option>
        `
    }
}
